...
return function(int $remessa, string $entidade, Environment $templates, array &$result): bool {
    ...
    $valor = \contab\tester\get_valor_manual($entidade, $remessa, 'rp_np_ultimo_exe');
    $etotal += $valor;
    $esquerdo['RP não processados inscritos no último exercício'] = $valor;
    
    $valor = \contab\tester\get_valor_manual($entidade, $remessa, 'rp_np_cancelado') * -1;
    $etotal += $valor;
    $esquerdo['(-) RP não processados cancelados'] = $valor;
    
    $valor = \contab\tester\get_valor_manual($entidade, $remessa, 'rp_np_liquidado') * -1;
    $etotal += $valor;
